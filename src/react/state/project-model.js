...
export const REACT_PROJECT_VERSION = 110;
...
export function createDefaultPlan() {
  ...
  const plan = {
    ...
    rooms: [
      ...
      polygonRoom("room-2", "Спальня", [
        [4.874, e],
        [8.486, e],
        [8.486, 3.374],
        [4.874, 3.374],
      ]),
      polygonRoom("room-3", "Спальня", [
        [e, 4.204],
        [3.174, 4.204],
        [3.174, 8.331],
        [e, 8.331],
      ...
      polygonRoom(
        ...
        { note: "помещение неправильной формы" },
      ),
      polygonRoom("room-5", "Котельная", [
        [4.874, 5.4],
        [8.486, 5.4],
        [8.486, 7.025],
        [4.874, 7.025],
      ]),
      polygonRoom("room-6", "Санузел", [
        [4.874, 7.025],
        [8.486, 7.025],
        [8.486, 9.126],
        [4.874, 9.126],
      ...
      polygonRoom("room-8", "Подсобка кухни", [
        [7.286, 9.884],
        [8.486, 9.884],
        [8.486, 12.801],
        [7.286, 12.801],
...
export function createCompactPlan() {
  ...
  plan.house = { w: 10, h: 7 };
  plan.rooms = [
    polygonRoom("compact-kitchen", "Кухня", [
      [e, e],
      [3.2, e],
      [3.2, 3.2],
      [e, 3.2],
    ]),
    polygonRoom("compact-bath", "Санузел", [
      [3.2, e],
      [5.2, e],
      [5.2, 2.45],
      [3.2, 2.45],
    ]),
    polygonRoom("compact-boiler", "Котельная", [
      [5.2, e],
      [7, e],
      [7, 2.45],
      [5.2, 2.45],
    ]),
    polygonRoom("compact-bed1", "Спальня 1", [
      [7, e],
      [9.826, e],
      [9.826, 3.45],
      [7, 3.45],
    ]),
    polygonRoom("compact-living", "Гостиная", [
      [e, 3.2],
      [5.2, 3.2],
      [5.2, 6.826],
      [e, 6.826],
    ]),
    polygonRoom("compact-hall", "Холл", [
      [3.2, 2.45],
      [7, 2.45],
      [7, 4.25],
      [5.2, 4.25],
    ...
    polygonRoom("compact-bed2", "Спальня 2", [
      [5.2, 4.25],
      [9.826, 4.25],
      [9.826, 6.826],
      [5.2, 6.826],
  ...
  return {
    ...
    settings: {
      ...
      roof: {
        ...
        showRafters: true,
        showLath: true,
        showCounterLath: false,
        includeEaveTrim: true,
...
export function createProjectWithCurrentPrices(currentProject) {
  ...
  if (Array.isArray(currentProject?.priceMat))
    next.priceMat = clone(currentProject.priceMat);
  if (Array.isArray(currentProject?.priceLab))
    next.priceLab = clone(currentProject.priceLab);
  return next;
}
...
export function normalizePlan(plan) {
  ...
  return {
    ...
    bindingLines: Array.isArray(plan.bindingLines)
      ? plan.bindingLines
...
const V51_FRAME_CATALOG_IDS = new Set([
  "MAT-013",
  "MAT-014",
  "MAT-015",
  "MAT-186",
...
function normalizeCatalog(items, defaults, upgradeIds = new Set()) {
  ...
  return normalized.concat(
    defaults.filter((item) => !existingIds.has(item.id)).map(clone),
  );
}
...
export function migrateProject(raw) {
  ...
  const upgradeV83Catalog = !Number.isFinite(savedVersion) || savedVersion < 83;
  const materialUpgradeIds = new Set([
    ...(upgradeFrameCatalog ? V51_FRAME_CATALOG_IDS : []),
    ...(upgradeV61Catalog ? V61_CATALOG_IDS : []),
    ...(upgradeV65Catalog
      ? [...V65_CATALOG_IDS].filter((id) => id.startsWith("MAT-"))
      : []),
    ...(upgradeV83Catalog
      ? [...V83_CATALOG_IDS].filter((id) => id.startsWith("MAT-"))
      : []),
  ]);
  const laborUpgradeIds = new Set([
    ...(upgradeV65Catalog
      ? [...V65_CATALOG_IDS].filter((id) => id.startsWith("LAB-"))
      : []),
    ...(upgradeV83Catalog
      ? [...V83_CATALOG_IDS].filter((id) => id.startsWith("LAB-"))
      : []),
  ]);
  const params = Array.isArray(raw.params)
    ? raw.params.map((entry) => Number(entry?.val) || 0)
    : [];
  const meta = raw.meta || {
    projectNum: raw.projectNum || "",
    customer: raw.cust || "",
    address: raw.addr || "",
    author: raw.author || "",
    date: raw.date || today(),
  ...
  return {
    ...
    estimateOverrides: Array.isArray(raw.estimateOverrides)
      ...
      : [],
    customEstimateLines: Array.isArray(raw.customEstimateLines)
      ? raw.customEstimateLines
      : [],
  };
}
