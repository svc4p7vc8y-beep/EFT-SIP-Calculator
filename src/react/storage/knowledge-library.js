...
function cleanImage(value) {
  return typeof value === "string" &&
    (value.startsWith("data:image/") || value.startsWith("./knowledge/"))
    ? value
    : "";
}
...
export function normalizeKnowledgeArticle(raw = {}) {
  ...
  const rows = Array.isArray(raw.table?.rows)
    ? raw.table.rows
        .slice(0, 100)
        .map((row) =>
          Array.isArray(row) ? row.slice(0, 4).map(cleanCell) : [],
        )
    : [];
  ...
  return {
    id:
      cleanCell(raw.id) ||
      `knowledge-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
    builtIn: false,
    ...
    tags: Array.isArray(raw.tags)
      ? raw.tags.map(cleanCell).filter(Boolean).slice(0, 12)
      : String(raw.tags || "")
          .split(",")
          .map(cleanCell)
          .filter(Boolean)
...
function fallbackRead() {
  try {
    return JSON.parse(localStorage.getItem(FALLBACK_KEY) || "[]").map(
...
function openDatabase() {
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(request.error || new Error("Не удалось открыть библиотеку"));
  });
}
...
async function withStore(mode, action) {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => resolve(request.result);
    request.onerror = () =>
      reject(request.error || new Error("Ошибка библиотеки"));
    transaction.oncomplete = () => database.close();
    transaction.onerror = () => {
      database.close();
      reject(transaction.error || new Error("Ошибка сохранения"));
    };
  });
}
...
export async function saveKnowledgeArticle(article) {
  const normalized = normalizeKnowledgeArticle({
    ...article,
  ...
  window.dispatchEvent(new CustomEvent("eft:knowledge-library-changed"));
  return normalized;
}
...
export async function replaceKnowledgeArticles(articles) {
  const normalized = (articles || []).map(normalizeKnowledgeArticle);
  try {
    const database = await openDatabase();
    await new Promise((resolve, reject) => {
      ...
      const store = transaction.objectStore(STORE);
      store.clear();
      normalized.forEach((article) => store.put(article));
      ...
      transaction.onerror = () => {
        database.close();
        reject(transaction.error || new Error("Ошибка синхронизации"));
      };
    });
  } catch {
    fallbackWrite(normalized);
  }
  return normalized;
}
...
export function getHiddenKnowledgeIds() {
  try {
    return JSON.parse(localStorage.getItem(HIDDEN_KEY) || "[]").filter(Boolean);
  } catch {
    return [];
  }
}
...
export function parseKnowledgeTransfer(raw) {
  if (raw?.format !== KNOWLEDGE_TRANSFER_FORMAT || !Array.isArray(raw.articles))
    throw new Error("Это не файл библиотеки знаний ЭФТ");
  if (raw.articles.length > 200)
    throw new Error("В файле слишком много материалов");
  return raw.articles.map((article, index) =>
    normalizeKnowledgeArticle({
      ...article,
      id: article.id?.startsWith("builtin-")
...
export function downloadKnowledgeTransfer(
  articles,
  fileName = "Библиотека_знаний_ЭФТ.eft-knowledge.json",
) {
  const blob = new Blob(
    [JSON.stringify(createKnowledgeTransfer(articles), null, 2)],
