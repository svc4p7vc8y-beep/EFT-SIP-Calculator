...
export const DEFAULT_FORMULAS = {
  ...
  partitionStudClearSpacingM: 0.59,
  partitionTopPlateLayers: 2,
  partitionOpeningExtraStuds: 2,
  partitionBoardStockLengthM: 6,
  ...
  sipBindingScrewSpacingM: 1.5,
  sipSupportBoardScrewSpacingM: 1.25,
  sipCornerScrewSpacingM: 1.5,
  sipEntranceOpeningFastenerMultiplier: 1.5,
  sipGarageOpeningFastenerMultiplier: 2,
  sipUniversalScrewsPerTNode: 2,
  sipUniversalScrewKgEach: 0.02,
  sipSealStaplesPerMeter: 16,
  sipSealStaplesPerPack: 1000,
  sipRoofSupportPointsPerPanel: 2,
  sipRidgePlateScrews: 8,
  ...
  sipStructuralScrewKg320: 0.1,
  pileConcreteM3: 0.01333,
  pileScrewKg: 0.12,
  pileLagScrews: 4,
  bindingPackScrewSpacingM: 0.5,
...
export function calculationFlowRows(project, calculation) {
  ...
  return [
    ...
    { group: 'Фундамент', source: 'Пакет досок обвязки', formula: `ceil(${calculation.foundation.bindingLength} м ÷ ${f.bindingPackScrewSpacingM} м) × ${Math.max(0, calculation.foundation.bindingLayers - 1)} соединяемых слоя`, result: Math.ceil(calculation.foundation.bindingLength / f.bindingPackScrewSpacingM) * Math.max(0, calculation.foundation.bindingLayers - 1), unit: 'шт', target: 'Саморезы 6×120 для сборки обвязки' },
    ...
    { group: 'СИП', source: calculation.sip.consumables.mode === 'quick' ? 'Площадь SIP-конструкций · сохранённый быстрый режим' : 'Опорные контуры пола/перекрытия/потолка, обвязки, углы, Т-узлы и наружные проёмы', formula: calculation.sip.consumables.mode === 'quick' ? `площадь × ${f.structuralFastenerKgPerM2} кг/м²` : `фактические узлы с шагами ${f.sipBindingScrewSpacingM} м / ${f.sipCornerScrewSpacingM} м; подробности — «Метизы по узлам»`, result: calculation.sip.consumables.mode === 'quick' ? calculation.sip.consumables.totals.structuralKg : calculation.sip.consumables.totals.structuralCount, unit: calculation.sip.consumables.mode === 'quick' ? 'кг' : 'шт', target: calculation.sip.consumables.mode === 'quick' ? 'Общая позиция «Саморезы конст.»' : 'Раздельные позиции 6×120, 8×180, 8×220, 8×280, 8×320 и проектные размеры' },
    { group: 'СИП', source: 'Т-образные узлы и опорный брус пола/потолка', formula: `${f.sipUniversalScrewsPerTNode} самореза на Т-узел; опорный брус с шагом ${f.sipSupportBoardScrewSpacingM} м`, result: calculation.sip.consumables.totals.universalScrewCount, unit: 'шт', target: 'Саморезы 6×120' },
    { group: 'СИП', source: project.settings.sip.partitionCalculationMode === 'area' ? 'Площадь каркасных перегородок · сохранённый старый режим' : 'Длина перегородок, стойки, нижняя и двойная верхняя обвязки, проёмы', formula: project.settings.sip.partitionCalculationMode === 'area' ? `площадь × ${f.partitionBoardM3PerM2} м³/м²` : `чистый шаг стоек ${f.partitionStudClearSpacingM} м; верхних обвязок ${f.partitionTopPlateLayers}; закупка по ${f.partitionBoardStockLengthM} м`, result: metrics.partitionLength, unit: 'м.п.', target: 'Каркас перегородок 50×100 / 50×150' },
    { group: 'СИП', source: 'Линии опирания и обвязки SIP', formula: `${calculation.sip.consumables.totals.sealLength} м × ${f.sipSealStaplesPerMeter} скоб/м`, result: calculation.sip.consumables.totals.stapleCount, unit: 'скоб', target: 'Уплотнительная лента 5 мм и скобы T53' },
    ...
    { group: 'Кровля', source: 'SIP-панели кровли и коньковые стыки', formula: `${f.sipRoofSupportPointsPerPanel} точки опирания на панель; ${f.sipRidgePlateScrews} саморезов на пластину`, result: calculation.roof.sipSupportScrewCount || 0, unit: 'конструкционных саморезов', target: `${calculation.roof.sipSupportScrewSize || 'по толщине панели'}; ${calculation.roof.sipRidgePlateCount || 0} коньковых пластин` },
    ...
    { group: 'Кровля', source: 'Фронтоны основной крыши и пристроек', formula: calculation.roof.flatSlopeMode === 'structural' ? `боковые треугольники ${calculation.roof.flatSideWallArea || 0} м² + высокая сторона ${calculation.roof.flatHighWallArea || 0} м²` : 'Σ основание × высота ÷ 2 × количество', result: calculation.roof.gableArea, unit: 'м²', target: 'Каркасные или SIP-фронтоны, облицовка и работы' },
    ...
    { group: 'Проёмы', source: 'Окна, двери и ворота всех этажей', formula: 'Изделия окон и ворот: ширина × высота; двери: поштучно', result: calculation.metrics.floorPlans.reduce((sum, item) => sum + (item.plan.openings || []).length, 0), unit: 'шт', target: 'Вычеты стен, изделия, монтаж' },
