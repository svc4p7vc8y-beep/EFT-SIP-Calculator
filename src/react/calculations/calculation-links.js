...
export function calculationFlowRows(project, calculation) {
  ...
  return [
    ...
    { group: 'СИП', source: calculation.sip.consumables.mode === 'quick' ? 'Площадь SIP-конструкций · сохранённый быстрый режим' : 'Опорные контуры пола/перекрытия/потолка, обвязки, углы и наружные проёмы', formula: calculation.sip.consumables.mode === 'quick' ? `площадь × ${f.structuralFastenerKgPerM2} кг/м²` : `опорные контуры и обвязки с шагом ${f.sipBindingScrewSpacingM} м + углы с шагом ${f.sipCornerScrewSpacingM} м + усиление проёмов`, result: calculation.sip.consumables.mode === 'quick' ? calculation.sip.consumables.totals.structuralKg : calculation.sip.consumables.totals.structuralCount, unit: calculation.sip.consumables.mode === 'quick' ? 'кг' : 'шт', target: calculation.sip.consumables.mode === 'quick' ? 'Общая позиция «Саморезы конст.»' : 'Конструкционные саморезы 8×180 / 8×220 / 8×280' },
