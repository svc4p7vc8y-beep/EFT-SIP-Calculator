...
import {
  ...
  pileRowAlignment, planIssues, rectanglePoints, roomPoints, roundCoord, snapPoint, unifiedWallSegments, withRoomBounds
...
export default function PlanScreen() {
  ...
  const toolHint = tool === 'select' ? 'Выберите и перетащите объект. Зелёные точки меняют его форму.' : tool === 'polygon' ? 'Щёлкайте по узлам комнаты и нажмите «Готово».' : tool === 'dimension' ? 'Протяните размер по объекту — линия автоматически вынесется наружу, затем её можно переместить.' : tool === 'pileRow' ? 'Начало и конец прилипают к узлам. Зелёный — точно по оси, красный — небольшое смещение.' : ['window', 'door', 'gap', 'pile'].includes(tool) ? 'Щёлкните место установки. Проёмы прилипнут к ближайшей стене.' : 'Зажмите кнопку мыши и протяните объект.';
