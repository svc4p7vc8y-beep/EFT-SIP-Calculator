...
function GarageGate({ opening, q, size }) {
  const offsets = [-0.25, 0, 0.25];
  if (opening.orientation === 'v') {
    return <g className="garage-gate">{offsets.map((offset) => <line key={offset} x1={q.x - 6} y1={q.y + size * offset} x2={q.x + 6} y2={q.y + size * offset} />)}</g>;
  }
...
function PlanCanvas({ plan, tool, selected, setSelected, commitPlan, polygonDraft, setPolygonDraft, issues }) {
  ...
  const toPlan = (event) => {
    ...
    if (current?.kind === 'endpoint') {
      const key = current.type === 'wall' ? 'walls' : current.type === 'dimension' ? 'dimensions' : 'pileRows';
  ...
  return <svg ref={svgRef} className={`plan-svg tool-${tool}`} viewBox={`0 0 ${VIEW.width} ${VIEW.height}`} role="img" aria-label="Редактор плана дома" onPointerDown={canvasDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={() => setGesture(null)}>
    ...
    {shownPlan.showPiles !== false ? (shownPlan.pileRows || []).map((row) => { const q = line(row); const selectedNow = selected?.type === 'pileRow' && selected.id === row.id; const spacing = Math.hypot(row.x2 - row.x1, row.y2 - row.y1) / Math.max(1, (row.count || 2) - 1); const alignment = pileRowAlignment(row); return <g key={row.id} className="planner-object" onPointerDown={(event) => objectDown(event, 'pileRow', row.id)}><line className={`pile-guide axis-${alignment.state} ${selectedNow ? 'selected' : ''}`} x1={q.a.x} y1={q.a.y} x2={q.b.x} y2={q.b.y} /><line className="wide-hit" x1={q.a.x} y1={q.a.y} x2={q.b.x} y2={q.b.y} /><text className={`pile-spacing-text axis-${alignment.state}`} x={(q.a.x + q.b.x) / 2} y={(q.a.y + q.b.y) / 2 - 9}>шаг {formatNumber(spacing)} м</text>{selectedNow ? [q.a, q.b].map((point, index) => <circle key={index} className="endpoint-handle" cx={point.x} cy={point.y} r="7" onPointerDown={(event) => objectDown(event, 'pileRow', row.id, { kind: 'endpoint', index })} />) : null}</g>; }) : null}
...
export default function PlanScreen() {
  ...
  const toolHint = tool === 'select' ? 'Выберите и перетащите объект. Окна, двери и ворота можно двигать прямо по стенам.' : tool === 'polygon' ? 'Щёлкайте по узлам комнаты и нажмите «Готово».' : tool === 'dimension' ? 'Протяните размер по объекту — линия автоматически вынесется наружу, затем её можно переместить.' : tool === 'pileRow' ? 'Начало и конец прилипают к узлам. Зелёный — точно по оси, красный — небольшое смещение.' : ['window', 'door', 'garage', 'gap', 'pile'].includes(tool) ? 'Щёлкните место установки. Проёмы прилипнут к ближайшей стене.' : 'Зажмите кнопку мыши и протяните объект.';
