...
import { NumberField, Panel, ResizableHeader, ScreenHeader, Stat, Toggle } from '../components/ui.jsx';
...
const FORMULA_GROUPS = [
  ...
  ['СИП и крепёж', [
    ...
    ['partitionStudClearSpacingM', 'Чистый шаг стоек перегородок', 'м'], ['partitionTopPlateLayers', 'Слоёв верхней обвязки перегородок', 'шт'],
    ['partitionOpeningExtraStuds', 'Дополнительных стоек на проём', 'шт'], ['partitionBoardStockLengthM', 'Складская длина доски перегородок', 'м'],
...
export default function CalculationSettingsScreen() {
  ...
  return <div className="screen calculation-settings-screen"><ScreenHeader title="Настройки расчёта" actions={<button className="button secondary" onClick={resetFormulas}><RotateCcw />Исходные коэффициенты</button>} />
    ...
    <Panel title="Полная расчётная ведомость" description="Все количества после формул и точная позиция прайса, из которой взята цена. Ширину колонок можно менять за правую границу заголовка."><div className="table-wrap formula-ledger"><table className="data-table resizable-table"><thead><tr>{['Раздел', 'Номенклатура', 'Источник', 'ID прайса', 'Количество', 'Цена', 'Сумма'].map(label => <ResizableHeader key={label}>{label}</ResizableHeader>)}</tr></thead><tbody>{calculation.lines.map((line) => <tr key={line.id}><td>{line.section}</td><td>{line.name}</td><td>{line.source}</td><td><code>{line.catalogId || 'НЕ НАЙДЕНО'}</code></td><td>{formatNumber(line.qty, 3)} {line.unit}</td><td>{formatMoney(line.price)}</td><td>{formatMoney(line.qty * line.price)}</td></tr>)}</tbody></table></div></Panel>
