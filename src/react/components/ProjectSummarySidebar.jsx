...
export default function ProjectSummarySidebar({ project, calculation, onNavigate }) {
  ...
  return <aside className="project-summary-sidebar" aria-label="Сводная ведомость проекта">
    ...
    <SummarySection title="Кровля" icon={Ruler} target="roof" onNavigate={onNavigate}>
      <div className="summary-grid"><SummaryValue label="Форма" value={ROOF_SHAPES[project.settings.roof.shape || 'gable']} /><SummaryValue label="Тип" value={ROOF_TYPES[project.settings.roof.type] || '—'} /><SummaryValue label="Стропила" value={(roof.rafterStructure?.section || project.settings.roof.rafterSection || '50x150').replace('x', '×')} /><SummaryValue label="Шаг" value={`${formatNumber(roof.rafterStructure?.step || .6)} м`} /><SummaryValue label="Доски 6 м" value={`${roof.rafterBoardCount || 0} шт`} /><SummaryValue label="Скаты" value={`${formatNumber((roof.coldSlopeArea || 0) + (roof.warmSlopeArea || 0))} м²`} /><SummaryValue label="Фронтоны" value={`${formatNumber(roof.gableArea)} м²`} /><SummaryValue label="Столбы террас" value={`${roof.terracePostCount || 0} шт`} /><SummaryValue label="Всего" value={`${formatNumber(roof.totalArea)} м²`} /></div><p className="summary-detail">{project.settings.roof.shape === 'flat' ? `Плоская кровля: длина ${formatNumber(project.settings.roof.ridgeLength)} м` : `Конёк: высота ${formatNumber(project.settings.roof.ridgeHeight)} м · длина ${formatNumber(project.settings.roof.ridgeLength)} м · ${roof.rafterStructure?.system === 'layered' ? 'наслонная' : 'висячая'} система`}</p>
    </SummarySection>
