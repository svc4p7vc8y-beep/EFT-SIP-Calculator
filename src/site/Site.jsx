...
function ProjectCard({ project, onOpen, index = 0 }) {
  return <button className="house-card motion-card" style={{ '--reveal-index': index }} aria-label={`Открыть проект ${project.name}`} onPointerEnter={() => import('./ProjectDialog.jsx')} onFocus={() => import('./ProjectDialog.jsx')} onClick={() => onOpen(project)}>
...
function ProjectCollection({ category, expanded, onOpen, onToggle }) {
  ...
  return <section className={expanded ? 'section projects-section project-collection is-expanded' : 'section projects-section project-collection'} id={category.id} data-reveal>
    ...
    <div className="catalog-toolbar"><span className="collection-count">{category.projects.length} проекта в подборке</span><button className="collection-open" aria-expanded={expanded} aria-controls={extraId} onClick={() => onToggle(category.id)}>{expanded ? 'Свернуть коллекцию' : 'Вся коллекция'}{expanded ? <ArrowDown className="turn-up" size={17} /> : <ArrowUpRight size={17} />}</button></div>
    <div className="houses-grid">{firstProjects.map((project, index) => <ProjectCard key={project.id} project={project} index={index} onOpen={(item) => onOpen({ ...item, category: category.title })} />)}</div>
    <div className="expanded-projects" id={extraId} aria-hidden={!expanded} inert={!expanded}><div className="houses-grid extra-houses-grid">{moreProjects.map((project, index) => <ProjectCard key={project.id} project={project} index={index + 2} onOpen={(item) => onOpen({ ...item, category: category.title })} />)}</div></div>
    {expanded ? <button className="collection-close-bottom" onClick={() => onToggle(category.id)}><ArrowDown className="turn-up" size={18} />Свернуть проекты</button> : null}
...
export default function Site() {
  ...
  function toggleCategory(categoryId) {
    ...
    if (closing) setTimeout(() => document.getElementById(categoryId)?.scrollIntoView({ behavior: window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 'instant' : 'smooth', block: 'start' }), 80);
  }
  ...
  return <><a className="skip-link" href="#main">К содержанию</a><header className="site-header"><a className="wordmark header-wordmark" href="#" aria-label="ЭФТ — на главную"><AnimatedLogo /><span className="brand-caption">Энергоэффективные<br />технологии</span></a><nav className={menuOpen ? 'site-nav open' : 'site-nav'} id="main-navigation" aria-label="Основная навигация">{nav.map(([id, label]) => <a className={activeNav === id ? 'active' : undefined} aria-current={activeNav === id ? 'location' : undefined} key={id} href={`#${id}`} onClick={() => { setActiveNav(id); setMenuOpen(false); }}>{label}</a>)}</nav><a className="btn outline header-cta" href="#contact" onClick={() => setActiveNav('contact')}>Обсудить проект<ArrowUpRight size={16} /></a><button className="icon-btn menu-toggle" aria-label={menuOpen ? 'Закрыть меню' : 'Открыть меню'} aria-expanded={menuOpen} aria-controls="main-navigation" onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? <X /> : <Menu />}</button><div className="header-build-line" aria-hidden="true" /></header>
    ...
    <main id="main"><section className="hero" data-reveal><div className="hero-copy"><h1>Дом, в котором<br />всё на своём<br /><em>месте.</em></h1><p>Продуманные пространства для жизни.<br />Проекты и СИП-панели от ЭФТ.</p><div className="hero-actions"><a className="btn primary" href="#projects">Выбрать проект<ArrowUpRight size={18} /></a><a className="btn outline" href="#panels">Купить СИП-панели</a></div><a className="hero-scroll" href="#projects"><span className="round-arrow"><ArrowDown size={18} /></span>Найдите место для своей истории</a></div><HeroPhoto /></section>
    ...
    <section className="approach-strip" id="about"><span className="section-number">Подход ЭФТ</span><p>Хороший проект начинается<br />с понимания <em>вашей задачи.</em></p><div>Сначала — привычки, пожелания и участок.<br />Затем — планировка и комплектация.<a className="text-link" href="./EFT_client_questionnaire.html">Расскажите о вашем проекте<ArrowUpRight size={18} /></a></div></section>
